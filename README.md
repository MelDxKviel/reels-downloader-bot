...
> ⚙️ Для работы второго сценария у бота в BotFather должен быть включён
> **inline feedback** (`/setinlinefeedback → 100%`) и включён сам inline-режим
> (`/setinline`). Whitelist действует и для inline-запросов: пользователи
> не из списка получат пустой ответ.
